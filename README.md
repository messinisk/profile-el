# profile-el
# 👋 Καλώς ήρθατε στο προφίλ μου

Είμαι ο Κωνσταντίνος Μεσσήνης και ασχολούμαι με διοίκηση επιχειρήσεων, ανάλυση δεδομένων, προγραμματισμό και συγγραφή μελετών. Το προφίλ αυτό λειτουργεί ως mini–portfolio που παρουσιάζει την εξέλιξή μου μέσα από πρακτικά έργα και τεκμηριωμένη δουλειά.

---

## 🎓 Σπουδές & Ακαδημαϊκό Υπόβαθρο

Πτυχιούχος Διοίκησης Επιχειρήσεων (Κατεύθυνση Περιφερειακής Ανάπτυξης)  
Τ.Ε.Ι. Ιονίων Νήσων  
Βαθμός: Λίαν Καλώς (6.69)  
Ημερομηνία Περάτωσης: 15/07/2025  

Η κατάρτισή μου συνδυάζει διοίκηση, οικονομία και ανάλυση δεδομένων, με έμφαση στη μεθοδολογία έρευνας και την κατανόηση κοινωνικοοικονομικών συστημάτων.

---

## 🧠 Δεξιότητες – Προγραμματισμός & Ανάλυση Δεδομένων

### 🔹 Τεχνικές δεξιότητες
- Python  
- R  
- Data cleaning & exploration  
- Βασική στατιστική  
- Τεκμηρίωση και οργάνωση project  

### 🔹 Πιστοποιήσεις
- ECDL – Microsoft Office  
- Coursera: Initiating and Planning Projects  
- Enterprise Greece: Εξαγωγικές διαδικασίες  

### 🔹 Repositories που αποδεικνύουν τις δεξιότητες
- Data Studies: https://github.com/messinisk/data-studies  
- Publications: https://github.com/messinisk/publications  
- UML & Architecture Models: https://github.com/messinisk/software_architecture_MODEL_UML  

---

## ✍️ Συγγραφικό Έργο

Έχω συγγράψει δοκίμια και μελέτες που συνδυάζουν κοινωνική ανάλυση, οικονομία και συστημική σκέψη.

### Δημοσιευμένα έργα
- Αναπάντητα Ερωτήματα  
- Κοινωνική Οικονομία  
- Αρχιτεκτονικών Model & UML Diagram  

### Repositories
- https://github.com/messinisk/Unanswered_Questions  
- https://github.com/messinisk/SOCIAL-ECONOMY  
- https://github.com/messinisk/publications  

---

## 📂 Επιλεγμένα Repositories – Πορεία & Εξέλιξη

### Κύριες αναφορές
- Data Studies
  - [Ανάλυση Μεγάλων Εταιρειών: Έσοδα, Απασχόληση και Κλάδοι](https://github.com/messinisk/data-studies/blob/main/100_Largest_companies/100_Largest_companies.md)
  - [Ανάλυση 200 εταιρείων σε  μεγέθους, ηλικίας και κλάδου δραστηριότητας](https://github.com/messinisk/data-studies/blob/main/200_Company_details/200_Company_details.md)
  - [Ανάλυση δεδομένων για την απάτη τραπεζικών λογαριασμών](https://github.com/messinisk/data-studies/blob/main/Bank%20Account%20Fraud%20Dataset%20Suite%20(NeurIPS%202022)/BankAccountFraudDatasetSuite.md)

- Publications  

### Δευτερεύουσες αναφορές
- Unanswered Questions  
- Social Economy  
- UML Architecture Models  

### Εξωτερικό Repo
- https://github.com/kostas30  

---

## 📫 Επικοινωνία
- GitHub: https://github.com/messinisk  
- Email: messiniskostas0@gmail.com  
